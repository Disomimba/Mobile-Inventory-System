...
if (aIsHoriz == bIsHoriz) continue; // parallel overlap, not a T/+ joint
...
bool cutVert; // true => split the vertical wall, false => split horizontal
...
if (vertContinuous && !horizContinuous) {
 ...
 // Default to splitting the horizontal wall so vertical depth is preserved.
 cutVert = false;
}
...
if (aIsHoriz != bIsHoriz) {
 ...
 double hExtend = horiz[1] - vert[1]; // How far the Horizontal wall extends Right (+X)
 double vExtend = vert[3] - horiz[3]; // How far the Vertical wall extends Down (+Y)
...
onMove: (details) {
 ...
 final Offset worldOffset = Offset(localOffset.dx + minX, localOffset.dy + minY);

 final Size previewSize = _getDefaultSize(details.data);
 final snappedPos = _snapToGrid(worldOffset, previewSize, details.data);
...
onAcceptWithDetails: (details) {
 ...
 final Offset finalPos = _snapToGrid(worldOffset, finalSize, details.data);
...
_selectedPopupElement = interactiveEl;
 ...
 setState(() {
_activeElementId = interactiveEl.id;
final double cosR = math.cos(interactiveEl.rotation);
final double sinR = math.sin(interactiveEl.rotation);
